...
public class EdgeWeightedSymbolGraph {
	 ...
	 * the names of the vertices adjacent with that vertex, separated by the
	 * delimiter.
	 * 
	 ...
	 * delimiter = args[1]; SymbolGraph sg = new SymbolGraph(filename, delimiter);
	 * Graph graph = sg.graph(); while (StdIn.hasNextLine()) { String source =
	 * StdIn.readLine(); if (sg.contains(source)) { int s = sg.index(source); for
	 * (int v : graph.adj(s)) { StdOut.println("   " + sg.nameOf(v)); } } else {
